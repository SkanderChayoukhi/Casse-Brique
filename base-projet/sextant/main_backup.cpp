...
extern "C" void Sextant_main(unsigned long magic, unsigned long addr)
{
	...
	// Simplified memory setup to avoid crashes
	mem_setup(&__e_kernel, (mbi->mem_upper << 10) + (1 << 20), &ecran);

	ecran.effacerEcran(NOIR);
	ecran.afficherMot("Sextant booted OK!", BLANC);
	ecran.sautDeLigne();
	...
	ecran.sautDeLigne();
	ecran.afficherMot("Checking PCI bus...", BLANC);
	ecran.sautDeLigne();

	// initialize pci bus to detect GPU address
	checkBus(0);

	ecran.afficherMot("PCI OK. Starting game...", VERT);
	ecran.sautDeLigne();

	// NO SCHEDULER - simple game loop only
	// irq_set_routine(IRQ_TIMER, sched_clk);

	game_start();

	// If game_start returns, hang here
	while (1)
		;
}
